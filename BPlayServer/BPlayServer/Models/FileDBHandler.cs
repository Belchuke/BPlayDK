...
namespace BPlayServer.Models
{
    ...
    {
        public static void GetMoviesFromDB(masterContext context)
        {
            ...
            using (Session thisSession = new Session())
            {
                ...
                List<Amovie> orderList = Items.OrderBy(o => o.ImdbRating).ToList();
                orderList.Reverse();
                orderList.RemoveAll(x => x.ImdbRating == "N/A");
                orderList.RemoveAll(x => x.Poster == "N/A");
               // list arr = Items.OrderBy(x => x.ImdbRating);
                try
                {
                    context.Amovie.AddRange(orderList);
                    context.SaveChanges();
                }
                catch (Exception ex)
                {
